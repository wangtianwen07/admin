...
public class RoleController extends CssController{
	...
	public String dir(Model model){
		...
		List<SSys> SSysAll = sysService.findAll();
		JSONObject sysJsonStr = new JSONObject();
		for (SSys sSys : SSysAll) {
			sysJsonStr.put(sSys.getUuid(),sSys.getName());
		}
		model.addAttribute("sysJsonStr",sysJsonStr);
